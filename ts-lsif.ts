...
import * as tss from 'lsif-tsc/lib/typescripts';

import { Id } from 'lsif-protocol';
import { Emitter, EmitterModule } from 'lsif-tsc/lib/emitters/emitter';
import { TypingsInstaller } from 'lsif-tsc/lib/typings';
import { lsif, ProjectInfo, Options as VisitorOptions } from 'lsif-tsc/lib/lsif';
import { Writer, StdoutWriter, FileWriter } from 'lsif-tsc/lib/utils/writer';
...
export function createEmitter(options: Options, writer: Writer, idGenerator: () => Id): Emitter {
	let emitterModule: EmitterModule;
	switch (options.outputFormat) {
		case 'json':
			emitterModule = require('lsif-tsc/lib/emitters/json');
			break;
		case 'line':
...
export function createIdGenerator(options: Options): () => Id {
	switch (options.id) {
		case 'uuid':
			return () => {
				return uuid.v4();
...
export async function processProject(config: ts.ParsedCommandLine, options: Options, emitter: Emitter, idGenerator: () => Id, typingsInstaller: TypingsInstaller): Promise<ProjectInfo | undefined> {
	...
	const host: ts.LanguageServiceHost = {
		...
		getScriptSnapshot: (fileName: string): ts.IScriptSnapshot | undefined => {
			...
			if (result === undefined) {
				...
				let content = (config as any).code
